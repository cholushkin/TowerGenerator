...
public class GroupsController : MonoBehaviour
{
    ...
    public void SetMaximizedFitRndConfiguration(Vector3 maxBBSize)
    {
        var origSeed = Seed;
        ...
        if (fitSeed == -1)
        {
            ...
            var bbxize = CalculateBBSize();
            var isInside = bbxize.x <= maxBBSize.x && bbxize.y <= maxBBSize.y && bbxize.z <= maxBBSize.z;
            Debug.LogError($"Can't fit even with minimal cfg resulting bb:{bbxize} in requested bb:{maxBBSize} - obj:{gameObject}");
            return;
        }
    ...
    // todo: build minimal impact tree from full hierarchy tree
    // void BuildMinimalGroupImpactTree

    public Vector2 GetAmmountOfGroups()
    {
        return new Vector2(transform.GetComponentsInChildren<Group>().Length, transform.GetComponentsInChildren<Group>(true).Length);
    }
}
